...
function populateCustomerDropdown() {
    ...
    const initialOptions = customers.map(c => ({
        ...
        text: `${c.customer_id} — ${c.outlet_name || 'Unspecified'}`,
        bde: c.bde || '',
        region: c.region || '',
        province: c.province || '',
        isNew: false
    ...
    tsInstances['field-customer'] = new TomSelect('#field-customer', {
        ...
        labelField: 'text',
        searchField: ['text'],
        placeholder: 'Search or type to add new Outlet...',
        dropdownParent: 'body',
        ...
        onChange: function (value) {
            ...
            if (value && value.startsWith('NEW::')) {
                ...
                if (tsInstances['field-bde']) {
                    ...
                    tsInstances['field-bde'].control_input.placeholder = 'Select BDE...';
                }

                clearTsField('field-bde');
                clearTsField('field-area');
            ...
            if (opt && !opt.isNew) {
                if (opt.bde) {
                    ...
                    setTsValue('field-bde', matchedUser ? matchedUser.user_id : opt.bde);
                } else {
                    clearTsField('field-bde');
                }
                setTsValue('field-area', opt.region || '');
                setTsValue('field-province', opt.province || '');
            } else {
                clearTsField('field-bde');
                clearTsField('field-area');
                clearTsField('field-province');
            ...
            if (tsInstances['field-bde']) {
                ...
                tsInstances['field-bde'].control_input.placeholder = 'Auto-filled from Outlet';
            }
        }
    });
}
